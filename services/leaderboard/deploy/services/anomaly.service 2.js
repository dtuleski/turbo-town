...
class AnomalyService {
    constructor(statisticsTableName = process.env.GAME_STATISTICS_TABLE_NAME || 'GameStatistics', leaderboardTableName = process.env.LEADERBOARD_TABLE_NAME || 'LeaderboardEntries') {
        // Velocity thresholds
        this.MAX_GAMES_PER_MINUTE = 10;
        this.MAX_GAMES_PER_DAY = 100;
        // Statistical outlier threshold (z-score)
        this.Z_SCORE_THRESHOLD = 3;
        // Pattern detection thresholds
        this.IDENTICAL_SCORE_THRESHOLD = 3; // Flag if same score appears 3+ times
        this.TIME_CONSISTENCY_THRESHOLD = 0.05; // Flag if completion times vary by less than 5%
     ...
     */
    async detectAnomalies(entry, gameStats) {
        const flags = {
        ...
        if (gameStats) {
            ...
            if (isOutlier) {
                ...
                flags.reasons.push(`Score ${entry.score} is a statistical outlier (>3 std dev from mean ${gameStats.mean.toFixed(2)})`);
            }
        }
        // Check velocity
        ...
        if (velocityCheck.isExcessive) {
            ...
            if (velocityCheck.gamesInLastMinute > this.MAX_GAMES_PER_MINUTE) {
                flags.reasons.push(`Excessive game velocity: ${velocityCheck.gamesInLastMinute} games in last minute`);
            }
            if (velocityCheck.gamesInLastDay > this.MAX_GAMES_PER_DAY) {
                flags.reasons.push(`Excessive game velocity: ${velocityCheck.gamesInLastDay} games in last day`);
            }
        }
        // Check patterns
     ...
     */
    isStatisticalOutlier(score, stats) {
        if (stats.stdDev === 0) {
     ...
     */
    async checkVelocity(userId, currentTimestamp) {
        ...
        try {
            ...
            const gamesInLastMinute = recentGames.filter((game) => new Date(game.timestamp) >= oneMinuteAgo).length;
            // Count games in last day
            const gamesInLastDay = recentGames.length;
            return {
                gamesInLastMinute,
                gamesInLastDay,
                isExcessive: gamesInLastMinute > this.MAX_GAMES_PER_MINUTE ||
        ...
        try {
            ...
            if (recentGames.length >= 3) {
                ...
                if (identicalScores.length >= this.IDENTICAL_SCORE_THRESHOLD) {
                    reasons.push(`Repeated identical score ${entry.score} appears ${identicalScores.length} times`);
                }
                ...
                if (completionTimes.length >= 3) {
                    ...
                    const variance = completionTimes.reduce((sum, time) => sum + Math.pow(time - mean, 2), 0) /
                        completionTimes.length;
                    const coefficientOfVariation = Math.sqrt(variance) / mean;
                    if (coefficientOfVariation < this.TIME_CONSISTENCY_THRESHOLD) {
                        reasons.push(`Suspiciously consistent completion times (CV: ${(coefficientOfVariation * 100).toFixed(2)}%)`);
                    }
                }
            }
        }
     ...
     */
    async getGameStatistics(gameType, difficulty) {
        try {
    ...
    async updateGameStatistics(gameType) {
        ...
        for (const difficulty of difficulties) {
            try {
                ...
                console.log(`Updated statistics for ${gameType} ${difficulty}: mean=${mean.toFixed(2)}, stdDev=${stdDev.toFixed(2)}, n=${scores.length}`);
            }
            catch (error) {
     ...
     */
    getDifficultiesForGameType(gameType) {
        switch (gameType) {
            case types_1.GameType.MEMORY_MATCH:
            case types_1.GameType.MATH_CHALLENGE:
            case types_1.GameType.WORD_PUZZLE:
                return [types_1.Difficulty.EASY, types_1.Difficulty.MEDIUM, types_1.Difficulty.HARD];
            case types_1.GameType.LANGUAGE_LEARNING:
                return [types_1.Difficulty.BEGINNER, types_1.Difficulty.INTERMEDIATE, types_1.Difficulty.ADVANCED];
            default:
                return [];
        }
    }
