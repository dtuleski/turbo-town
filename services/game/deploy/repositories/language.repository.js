...
class LanguageRepository {
    ...
    async getAllLanguageWords() {
        try {
            logger_1.logger.info('Getting all language words for admin');
            const command = new lib_dynamodb_1.ScanCommand({
                TableName: this.wordsTableName
            });
            const result = await dynamodb_1.docClient.send(command);
            ...
            logger_1.logger.error('Failed to get all language words', error);
            throw error;
        }
    }
    /**
     ...
     */
    async getLanguageWordById(wordId) {
        try {
            logger_1.logger.info('Getting language word by ID', { wordId });
            const command = new lib_dynamodb_1.GetCommand({
                TableName: this.wordsTableName,
                Key: { wordId }
            });
            ...
            logger_1.logger.error('Failed to get language word by ID', error, { wordId });
            throw error;
        }
    }
    /**
     ...
     */
    async updateLanguageWord(wordId, updates) {
        try {
            logger_1.logger.info('Updating language word', { wordId, updates });
            ...
            logger_1.logger.error('Failed to update language word', error, { wordId });
            throw error;
        }
    }
    /**
