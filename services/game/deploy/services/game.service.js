...
class GameService {
    ...
    async completeGame(userId, input, username) {
        ...
        // Calculate accuracy (perfect score = 1.0, decreases with more attempts)
        const maxAttempts = game.difficulty * 2; // Rough estimate
        const accuracy = Math.max(0, Math.min(1, 1 - (input.attempts - game.difficulty) / maxAttempts));
        ...
        const avgResponseTimeSeconds = Math.max(1, Math.round(completedGame.completionTime / input.attempts));
        ...
        await this.eventPublisher.publishGameCompleted({
            ...
            userId: completedGame.userId,
            username: username,
            gameType: 'MEMORY_MATCH', // Default game type
            ...
            performanceMetrics: {
                attempts: input.attempts,
                hintsUsed: 0, // Not tracked yet
                pauseCount: 0, // Not tracked yet
