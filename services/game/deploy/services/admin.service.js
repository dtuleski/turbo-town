...
class AdminService {
    ...
    async getAdminAnalytics() {
        ...
        const gamesToday = allGames.filter((g) => g.startedAt?.startsWith(today));
        const gamesThisWeek = allGames.filter((g) => g.startedAt >= weekAgo);
        const gamesThisMonth = allGames.filter((g) => g.startedAt >= monthAgo);
        ...
        const dauUsers = new Set(gamesToday.map((g) => g.userId));
        const dau = dauUsers.size;
        // Calculate MAU estimate (unique users this month)
        const mauUsers = new Set(gamesThisMonth.map((g) => g.userId));
        ...
        const avgGamesPerUser = totalUsers > 0
            ...
            : 0;
        // Users by tier
        const tierCounts = { FREE: 0, LIGHT: 0, BASIC: 0, STANDARD: 0, PREMIUM: 0 };
        const tierGames = { FREE: [], LIGHT: [], BASIC: [], STANDARD: [], PREMIUM: [] };
        // Map subscriptions
        const userTiers = {};
        ...
        const topUsers = Object.entries(gamesPerUser)
            ...
            const user = cognitoUsers.find((u) => u.userId === userId);
            const rateLimit = allRateLimits.find((r) => r.userId === userId);
            return {
                userId,
    ...
    async listAllUsers(input) {
        ...
        const users = cognitoUsers.map((user) => {
            const userGames = allGames.filter((g) => g.userId === user.userId);
            const rateLimit = allRateLimits.find((r) => r.userId === user.userId);
            const subscription = allSubscriptions.find((s) => s.userId === user.userId);
            const tier = subscription?.tier || 'FREE';
            const todayGames = userGames.filter((g) => g.startedAt?.startsWith(today));
            const lastGame = userGames.sort((a, b) => (b.startedAt || '').localeCompare(a.startedAt || ''))[0];
            const tierLimits = {
                FREE: 3,
                LIGHT: 20,
                BASIC: 20,
    ...
    calculateTierRevenue(tier, count) {
        const prices = {
            FREE: 0,
            BASIC: 1.99,
            PREMIUM: 9.99
